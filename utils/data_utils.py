...
DATA_ROOT_ABS = os.path.join(base_path, 'data')
        ...
        if filename is None:
            if train:
                if percent >= 0.1:
                    ...
                    self.images, self.labels = np.load(os.path.join(data_path, 'partitions/train_part5.pkl'), allow_pickle=True)
...
class OfficeDataset(Dataset):
    def __init__(self, base_path, site, train=True, transform=None):
        if train:
            self.paths, self.text_labels = np.load(os.path.join(DATA_ROOT_ABS, 'office_caltech_10', '{}_train.pkl'.format(site)), allow_pickle=True)
        else:
            self.paths, self.text_labels = np.load(os.path.join(DATA_ROOT_ABS, 'office_caltech_10', '{}_test.pkl'.format(site)), allow_pickle=True)
...
class DomainNetDataset(Dataset):
    def __init__(self, base_path, site, train=True, transform=None):
        if train:
            self.paths, self.text_labels = np.load(os.path.join(DATA_ROOT_ABS, 'DomainNet', '{}_train.pkl'.format(site)), allow_pickle=True)
        else:
            self.paths, self.text_labels = np.load(os.path.join(DATA_ROOT_ABS, 'DomainNet', '{}_test.pkl'.format(site)), allow_pickle=True)
